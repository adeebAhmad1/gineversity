...
import { registerFeeds, getFeeds, getFeedsTopic } from '../../redux/actions/feedActions';
...
class index extends Component {
    ...
    componentDidMount() {
        const { getFeeds, tId, getFeedsTopic } = this.props;
        if (tId) {
            var data = { tId: tId, limit: 0 };
            getFeedsTopic(data, 0);
        } else getFeeds(0, 0);
    }


    componentDidUpdate(prevP, prevS) {
        const { tId , getFeedsTopic} = this.props;
        if (prevP.tId !== tId) {
            var data = { tId: tId, limit: 0 };
            getFeedsTopic(data, 0);
        }
    ...
    renderCommunityLinks(TopicList) {
        return TopicList && TopicList.length > 0 && TopicList.map(i => {
            return <li key={i._id} className="side-item side-item-hotel">
                <Link to={`/community/${i._id}`} className="side-link">
    ...
    renderCoursesLinks(TopicList) {
        return TopicList && TopicList.length > 0 && TopicList.slice(0, 2).map(i => {
...
export default connect(mapStateToProps, { registerFeeds, getFeeds, getFeedsTopic })(index);
